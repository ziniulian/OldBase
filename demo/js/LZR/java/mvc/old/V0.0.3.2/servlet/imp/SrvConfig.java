...
public class SrvConfig implements InfSrv {
	 ...
	 * 默认包名
	 */
	private String defaultPackageName = "lzr.java.mvc.dao.config.sql.";

	/**
	 * 默认类名
	 */
	private String defaultClassName = "lzr.java.mvc.dao.config.sql.SqlConfig";
		...
		if (null==method || methodRange.indexOf(method) == -1) {
			...
			if (method.equals("fun")) {
				...
				if (method.equals("url")) {
					...
					sr.setData( Ctrl.json.toMap(obj) );
					r = sr.doPost(false);
				} else {
