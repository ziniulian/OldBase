...
LZR.HTML5.Canvas.Thumbnail.prototype.version = "0.0.0";
...
// 更新鼠标事件
LZR.HTML5.Canvas.Thumbnail.prototype.ctrlUpdate = function() {
	var s;
	if (this.ctrl.state == this.ctrl.STATE.LEFT) {
		if ( this.direction === 1 ) {
			s = this.ctrl.leftStart.x - this.ctrl.leftEnd.x;
		} else if ( this.direction === 2 ) {
			s = this.ctrl.leftStart.y - this.ctrl.leftEnd.y;
		}
		if (s!==0) {
		...
		s = this.ctrl.wheelValue * -this.wheelScale;
...
*/
		// 动画总帧数
		this.alineShow (dp, 5, 0, 20);
	}
};

// 归位动画
LZR.HTML5.Canvas.Thumbnail.prototype.alineShow = function (dp, num, current, waitTime) {
	/*
		参数说明：
			dp：目标位置
			num：动画总帧数
...
*/
	...
	if ( t<=1 ) {
		...
		if (d === 0) {
			...
			this.flush();
			current++;
			setTimeout( LZR.HTML5.Util.bind (this, this.alineShow, dp, num, current, waitTime) ,waitTime );
		}
	}
};
