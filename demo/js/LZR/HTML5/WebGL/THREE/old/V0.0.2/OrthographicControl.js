...
LZR.HTML5.loadJs([
	LZR.HTML5.jsPath + "HTML5\\expand\\threejs\\three.js",
	LZR.HTML5.jsPath + "HTML5\\util\\MouseDropController.js",
	LZR.HTML5.jsPath + "util\\Graphics\\CoordinateChange.js"
]);
LZR.HTML5.WebGL.Three.OrthographicControl = function (obj) {
/*
	参数说明：
	{
		obj.ctrlDom,				// 控制对象
...
*/
	...
	LZR.HTML5.Util.MouseDropController.call(this, obj.ctrlDom);
	...
	// 光源
	if (obj.light) {
		if (LZR.HTML5.Util.getClassName(obj.light) == "Array") {
	...
	this.resetter = {
		...
		up: null,
		zoom: 0,
		lights: LZR.HTML5.Util.clone(this.lights)
	};
...
LZR.HTML5.WebGL.Three.OrthographicControl.prototype = Object.create(LZR.HTML5.Util.MouseDropController.prototype);
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.className = "LZR.HTML5.WebGL.Three.OrthographicControl";
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.version = "0.0.2";

// 初始化
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.init = function(obj) {
	this.initCamera();
};

// 初始化相机位置
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.initCamera = function() {
...
/*
// 测试用，显示换算结果。
LZR.HTML5.log(	"position : " + this.rigid.position.x + " , " + this.rigid.position.y + " , " + this.rigid.position.z + "\n" +
...
// 缩放相机
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.zoom = function () {
	// 计算缩放比例
	...
	if (zs) {
		// 恢复鼠标位置
		this.wheelValue = 0;
		this.midStart = LZR.HTML5.Util.clone(this.midEnd);
...
// 平移相机
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.pan = function () {
	...
	if (x || y) {
		...
		// 向量旋转
		if (!this.cc) {
			this.cc = new LZR.Util.Graphics.CoordinateChange({
				s: {		// 相机坐标系
					p1: {x:0, y:0, z:0},
					p2: {x:1, y:0, z:0},
					p3: {x:0, y:1, z:0},
		...
		// 恢复鼠标位置
		this.rightStart = LZR.HTML5.Util.clone(this.rightEnd);
	}
};

// 旋转相机
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.rotate = function () {
	if (!this.ball) {
		this.ball = LZR.HTML5.Util.getDomPositionForDocument(this.ctrlDom);
	}
...
// 5. 旋转：误差最大，必须消除。
LZR.HTML5.log(	"（涉及旋转）axis.L : " + axis.length() + "\n" +
...
*/
		...
		// 恢复鼠标位置
		this.leftStart = LZR.HTML5.Util.clone(this.leftEnd);
	}
};

// 换算球面坐标
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.getCoordinateOnBall = function (p) {
...
// 更新
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.update = function() {
	switch (this.state) {
...
// 重置
LZR.HTML5.WebGL.Three.OrthographicControl.prototype.reset = function() {
