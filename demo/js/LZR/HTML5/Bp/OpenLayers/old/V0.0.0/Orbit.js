...
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.version = "0.0.0";
	...
	for (var i=0; i<this.data.length; i++) {
		if (this.data[i].visible) {
			...
			if (this.orbitSelectedIndex === i || this.orbitOverIndex === i) {
				// 画节点
				ctx.fillStyle = this.nodeColor[i];
				ctx.fill();
				...
				if (this.nodeOverIndex >= 0) {
					this.drawNode (nodes[this.nodeOverIndex], ctx, i);
				}
			} else {
				// 画轨迹动画
...
// 生成节点路径
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.createNodePath = function (nodes, ctx) {
	ctx.beginPath();
...
// 画节点动画
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.drawNode = function (node, ctx, index) {
	...
	ctx.fillText (node.time, node.x + this.nodeLen, node.y);
};
