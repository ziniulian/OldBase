...
// console.log(this.orbitSelectedColor);
		...
		// 轨迹发光颜色
		this.orbitLightColor = this.initColor(obj.orbitLightColor);
...
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.version = "0.0.2";
...
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.flush = function (ctx) {
	...
	for (var i=0; i<this.data.length; i++) {
		if (this.data[i].visible) {
			...
			// 生成节点路径
			this.getNodePath (nodes, ctx, 0);

			// 判断轨迹是否经过
			if (this.orbitSelectedIndex === i || this.orbitOverIndex === i || this.orbitOverIndex === -1) {
			...
			// 画发光路径
			ctx.shadowColor = this.orbitLightColor[i];
			if (this.orbitOverIndex === i) {
				this.drawLightOrbit (nodes, ctx, ctx.strokeStyle);
			}

			if (this.orbitSelectedIndex === i || this.orbitOverIndex === i) {
				// 画节点
				this.drawNode (nodes, ctx, this.nodeColor[i], true);
				...
				// 画节点
				if (this.showAllNode) {
					this.drawNode (nodes, ctx, this.nodeColor[i], false);
				}
			}
...
// 画发光轨迹
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.drawLightOrbit = function (nodes, ctx, cf) {
	ctx.shadowBlur = this.orbitBlur;
	ctx.fillStyle = cf;

	var w = this.nodeWidth/2;
	for (i=1; i<nodes.length; i++) {
		ctx.beginPath();
		ctx.moveTo(nodes[i].x-w, nodes[i].y);
		ctx.lineTo(nodes[i-1].x-w, nodes[i-1].y);
		ctx.lineTo(nodes[i-1].x+w, nodes[i-1].y);
		ctx.lineTo(nodes[i].x+w, nodes[i].y);
		ctx.closePath();
		ctx.fill();
	}
	ctx.shadowBlur = 0;
...
// 获取节点路径
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.getNodePath = function (nodes, ctx, showNode) {
	ctx.beginPath();
	var w = 2*this.nodeWidth;
	for (var i=0; i<nodes.length; i++) {
		ctx.rect(nodes[i].x-this.nodeWidth, nodes[i].y-this.nodeWidth, w, w);
	}
};

// 画节点
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.drawNode = function (nodes, ctx, c, light) {
...
LZR.HTML5.Bp.OpenLayers.Orbit.prototype.drawNodeAnimation = function (node, ctx, index) {
	...
	ctx.arc(node.x, node.y, nodeW, 0, 2*Math.PI);
	ctx.stroke();
