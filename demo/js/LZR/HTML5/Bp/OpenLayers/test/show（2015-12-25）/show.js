...
var show = {
	...
	init: function() {
		...
		// CityAndStation_date.value = this.timeToFomQryString(this.addHour(-24)).substring(0, 10);
		CityAndStation_date.value = "2015-11-11";
		LZR.HTML5.Util.Event.addEvent (CityAndStation_date, "change", hw.bind(this, function() {
			this.md.m_time.productTime.set(LZR.Util.Date.getDate(CityAndStation_date.value + " " + CityAndStation_tim.value));
		}), false);
		LZR.HTML5.Util.Event.addEvent (CityAndStation_tim, "change", hw.bind(this, function() {
			this.md.m_time.productTime.set(LZR.Util.Date.getDate(CityAndStation_date.value + " " + CityAndStation_tim.value));
		}), false);
		LZR.HTML5.Util.Event.addEvent (CityAndStation_mod, "change", hw.bind(this, function() {
			this.md.mod.set(CityAndStation_mod.value);
		}), false);
	...
	buildModel: function() {
		...
		this.md.ctrl.date = CityAndStation_date;
		this.md.ctrl.tim = CityAndStation_tim;
		this.md.ctrl.mod = CityAndStation_mod;
		this.md.ctrl.date.out = CityAndStation_dateOut;
		this.md.ctrl.tim.out = CityAndStation_timOut;
		this.md.ctrl.mod.out = CityAndStation_modOut;
	...
	// 获取所有城市
	getCitys: function() {
		this.aj.get("data/Citys", hw.bind(this, this.handleCitys));
	},

	// 处理城市
	handleCitys: function(txt) {
		var data = JSON.parse(txt);
// var a = document.createElement("a");
// a.innerText = "城市数据 &nbsp ";
// a.download = "Citys";
// var blob = new Blob([JSON.stringify(data)], {type:"text/text;charset=utf-8"});
// var u = URL.createObjectURL(blob);
// a.href = u;
// this.md.ctrl.appendChild(a);
	...
	getFom: function() {
		...
		if (par.productDate == "2015-11-11 20:00:00" && par.modeName == "naqpms") {
			this.aj.get("data/Foms", hw.bind(this, this.handleFom));
		} else {
			this.aj.get("data/Foms_null", hw.bind(this, this.handleFom));
		}
	},
...
// this.md.ctrl.appendChild(a);
	...
	// 关闭右边图表
	closeRight: function () {
		// topic.publish("closeRightPanelWidget");
		this.md.m_time.ctrl.init();
	}
