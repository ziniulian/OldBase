...
LZR.HTML5.Bp.AirqMg.RegStat.prototype.version = "0.0.4";
...
// 加载风场
LZR.HTML5.Bp.AirqMg.RegStat.prototype.loadWindContour = function (url) {
	if (!url) {
		url = this.windContourPath;
	}
	this.maps.add(url, -1, LZR.HTML5.Util.bind ( this, function (index, img) {
		var c = new LZR.HTML5.Canvas.Layer({name:"风场", obj:img});
		c.visible = false;
		for (var i=0; i<this.tbn.count; i++) {
			this.tbn.imgs[i].layers.push( c );
		}
		// this.tbn.onchange (this.tbn.index);
console.log("wind_OK");
...
// 加载湿度
LZR.HTML5.Bp.AirqMg.RegStat.prototype.loadRhContour = function (url) {
	if (!url) {
		url = this.rhContourPath;
	}
	this.maps.add(url, -2, LZR.HTML5.Util.bind ( this, function (index, img) {
		var c = new LZR.HTML5.Canvas.Layer({name:"湿度", obj:img});
		c.visible = false;
		for (var i=0; i<this.tbn.count; i++) {
			this.tbn.imgs[i].layers.push( c );
		}
...
// 加载气压
LZR.HTML5.Bp.AirqMg.RegStat.prototype.loadPrContour = function (url) {
	if (!url) {
		url = this.prContourPath;
	}
	this.maps.add(url, -3, LZR.HTML5.Util.bind ( this, function (index, img) {
		var c = new LZR.HTML5.Canvas.Layer({name:"气压", obj:img});
		c.visible = false;
		for (var i=0; i<this.tbn.count; i++) {
			this.tbn.imgs[i].layers.push( c );
		}
...
console.log("pr_OK");
		...
		// 加载温度
		this.loadTempContour();
	} ));
};

// 加载温度
LZR.HTML5.Bp.AirqMg.RegStat.prototype.loadTempContour = function (url) {
	if (!url) {
		url = this.tempContourPath;
	}
	this.maps.add(url, -4, LZR.HTML5.Util.bind ( this, function (index, img) {
		var c = new LZR.HTML5.Canvas.Layer({name:"温度", obj:img});
		c.visible = false;
		for (var i=0; i<this.tbn.count; i++) {
			this.tbn.imgs[i].layers.push( c );
		}
...
// 多图片同时下载测试
LZR.HTML5.Bp.AirqMg.RegStat.prototype.loadTest = function () {
	console.log ("多图测试：Start");
	var d = {
		type: "picContent",
		picType: ["35"],
		sort: 0,
	...
	var callback = function (i) {
		return LZR.bind (this, function (i, index, img, data) {
			// console.log ("多图测试：" + i + "-" + index + " : " + data.name);
		}, i);
	};

	var finish = function (i) {
		return LZR.bind(this, function (i) {
			console.log ("多图测试：" + i + " : End");
		}, i);
	};
...
// var r = LZR.HTML5.Util.clone(p);
// console.log ("r = " + JSON.stringify (r));
		var md = new LZR.HTML5.Canvas.ImgLoader(callback(i));
		md.addByWebSocket (this.webSocketUrl, LZR.HTML5.Util.clone(d));
		md.finished = finish(i);
	}
	console.log ("多图测试：End");
};
