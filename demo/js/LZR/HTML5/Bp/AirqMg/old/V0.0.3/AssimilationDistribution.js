...
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.version = "0.0.3";
...
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.initTbn = function () {
	if (this.condition.ttyp == 1) {
		this.tbn.index = this.condition.count - 1;
		this.tbn.count = this.condition.count;
...
// 启动循环控制
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.ctrlStart = function () {
	this.map.ctrlEnable();
	this.tbn.ctrlEnable();
	this.eys.ctrlEnable();
...
// 关闭循环控制
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.ctrlStop = function () {
	this.ctrlEnable = 0;
...
// 循环控制
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.ctrl = function (v, start) {
...
// 生成图层下载器
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.createLayersLoader = function () {
...
// 填充图层
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.fillLayers = function (img) {
	var i , j, ay;
...
// 生成气象图查询条件
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.createWeatherQry = function (qry, typ, mod) {
	qry.picType = [typ];
	qry[typ] = {
		"modelType": [mod],
		"domain": ["d"+ LZR.HTML5.Util.format (this.condition.area, 2, "0")],
...
// 播放动画
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.play = function (v) {
	if (this.playState == 1) {
		if (v>0) {
			v--;
		} else {
...
// 动画控制
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.changeAnimation = function () {
	if (this.playState == -1) {
		return 0;
...
// 设置跳帧
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setFrame = function (i) {
	this.playFrame = i;
};

// 控制帧头帧尾
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.changeHeadTail = function (i) {
	if (i) {
...
// 控制翻页
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.changePage = function (i) {
	this.tbn.select(this.tbn.index + i * this.playFrame);
};

// 设置播放速度
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setSpeed = function (i) {
	this.playSpeed += i;
...
// 清空图层中的图片
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.clrLayers = function (index) {
...
// 设置模式
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setMod = function (value, redo) {
...
// 设置污染物
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setFom = function (name, value, redo) {
...
// 设置区域（预报天数、比例坐标会变化）
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setArea = function (name, value, redo, dn, ra) {
...
// 设置预报天数
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setDayNum = function (dn) {
	// 重设预报天数
	if (dn) {
		this.dayNum = dn;
	}
...
// 设置比例尺范围
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setRulerArea = function (ra) {
	// 重设比例尺范围
	if (ra) {
		if (!isNaN(ra.latt)) {
...
// 设置图层可见
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setLayerVisble = function (index, visible) {
	for (var i = 0; i<this.tbn.count; i++) {
...
// 设置图层透明度
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setLayerAlpha = function (index, alpha) {
	if ( !isNaN(alpha) ) {
		if (alpha < 0) {
			alpha = 0;
		} else if (alpha > 1) {
...
// 加载指南针
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.loadCompass = function (compassPath) {
...
// 设置指南针是否可见
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setCompassVisble = function (visible) {
	if (this.compass.img) {
...
// 加载图例
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.loadLegend = function (legendPath) {
	// 创建图例对象
...
// 设置图例图片
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setLegendImg = function (path) {
...
// 设置图例的鼠标经过时的图片
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setLegendOverImg = function (path) {
...
// 设置图例是否可见
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.setLegendVisble = function (visible) {
	if (this.legend.img) {
...
// 生成比例尺
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.createRuler = function () {
...
// 生成滚动条
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.createScroll = function (p) {
	if (!this.scroll) {
...
// 刷新滚动条
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.fulshScroll = function () {
...
// 窗体自适应功能
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.resize = function () {
	this.map.resize();
	this.tbn.resize();
	this.eys.resize();
	this.loadEys();
...
// 地图额外画图
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.mapAutoFlush = function (ctx, x, y, w, h) {
	var width, height;
	ctx.globalAlpha = 1;
...
// 返回某个图层加载器是否繁忙
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.isBusyInLayersLoader = function (index) {
...
// 其它图层加载完时的接口
LZR.HTML5.Bp.AirqMg.AssimilationDistribution.prototype.otherFinished = function (id) {};
