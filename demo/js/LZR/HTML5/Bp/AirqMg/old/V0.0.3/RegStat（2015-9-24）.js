...
// ----------- 区域形势 ------------
/*
	原V0.0.3版基础上修改文件路径，以匹配图片接口的标准路径。（2015-9-24）
*/
...
// LZR.HTML5.log(new Date (this.condition.date));
	var head = this.path + "pic/figure/Pol/ConcSpa/";
	var url = "_d0";

	// 污染物
	var path = this.condition.fom;		// PM25HourlySpa_d03_NAQPMS_2015061720_171
	path += '/';
	...
	ts += this.condition._tim;

	path += ts;
	path += '/';
	path += this.condition.fom;
	url += ts;

	for (var i=0; i<this.tbn.count; i++) {
		var r = this.createImg (i, this.condition.date, path, url, head);
...
// 生成图片信息
LZR.HTML5.Bp.AirqMg.RegStat.prototype.createImg = function (index, d, path, url, head) {
	var r = new LZR.HTML5.Bp.AirqMg.RegImg();
	if (this.condition.ttyp == 1) {
		r.url = head + "Hourly/" + path + "HourlySpa" + url + "_";
		...
		r.url = head + "Daily/" + path + "DailySpa" + url + "_";
