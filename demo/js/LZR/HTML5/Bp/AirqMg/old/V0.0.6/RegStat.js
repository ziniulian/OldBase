...
LZR.HTML5.Bp.AirqMg.RegStat = function (obj) {
	...
	*/
	...
	this.tbn.onchange = LZR.HTML5.Util.bind (this, function (i) {
		...
		// OpenLayers 图层切换
		for (var k=0; k<this.layersNum; k++) {
			var s = this.tbn.imgs[i].getLayers()[k].source;
			if (s) {
				this.olLayers[k].setSource(s);
			}
		}
	});
...
LZR.HTML5.Bp.AirqMg.RegStat.prototype.version = "0.0.6";
...
LZR.HTML5.Bp.AirqMg.RegStat.prototype.createLayersLoader = function () {
	...
	// 生成 OpenLayers 图层组
	this.olLayers = [];		// OpenLayers 图层组
	for (i=0; i<this.layersNum; i++) {
		this.olLayers.push( new ol.layer.Image({
			visible: false
		}) );
		if (this.olmap) {
			this.olmap.addLayer(this.olLayers[i]);
		}
	}
	this.olLayers[0].setVisible(true);
};
...
/*
	// OpenLayers 叠图分辨率测试
	var i, ps = [];
	var psurl = "http://192.168.1.101/imgServer/TestOpenLayers/PM25HourlySpa_d01_NAQPMS_2015070320_0";
	for (i=4; i<31; i++) {
		ps[i] = psurl + LZR.HTML5.Util.format (i, 2, "0") + ".png";
	}
	for (i=0; i<ps.length; i++) {
console.log (ps[i]);
		this.layersLoader[id].add(ps[i], i);
	}
*/
};
...
LZR.HTML5.Bp.AirqMg.RegStat.prototype.onLayersLoad = function (id, index, img) {
	...
	if (ms) {
		...
		// 生成OpenLayers图层的 source
		ms.layers[id].source = new ol.source.ImageStatic({
			url: img.src,
			imageExtent: ol.proj.transformExtent ([this.rulerArea.lonl, this.rulerArea.latb, this.rulerArea.lonr, this.rulerArea.latt], ol.proj.get('EPSG:4326'), ol.proj.get('EPSG:3857'))
		});
		if (id===0 && index === this.tbn.index) {
...
LZR.HTML5.Bp.AirqMg.RegStat.prototype.setLayerVisble = function (index, visible) {
	...
	// OpenLayers 图层修改
	this.olLayers[index].setVisible(visible);
};
...
LZR.HTML5.Bp.AirqMg.RegStat.prototype.setLayerAlpha = function (index, alpha) {
	...
	// OpenLayers 图层修改
	this.olLayers[index].setOpacity(alpha);
};
