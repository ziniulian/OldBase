...
var Lzrut = {
	...
	targThis: function (thisObj) {
		var classObj = Lzrut.getClassObj(thisObj);
		if (!classObj.lzrTarg) {
	...
	getThis: function (newObj) {
		return Lzrut.getClassObj(newObj).lzrTarg;
	},

	// 保持 function 的 this 指針（Demo 1.00版）
		/** 用法：
		 *	某类.prototype.某方法 = function () {
		 *		var caller = new ThisTag(this);	// 创建该实例
		 *
		 *		......  其它程序码  .........
		 *		
		 *		// 设置某回调函数
	...
	addWheel: function(obj, callback, useCapture) {
		...
		return Lzrut.addEvent(obj, wheelType, function(e){
			var event = Lzrut.getEvent(e);
